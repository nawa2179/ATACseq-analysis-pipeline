...
DESeq2_DE_res = function(Exp, dds_res, pval, FC, i, j, correction, record){
  ...
  volcano1 <- EnhancedVolcano(res_ij,lab = rownames(res_ij), x = 'log2FoldChange', y = 'pvalue', xlim = c(-5, 8))
  volcano1
  ...
  volcano2 <- EnhancedVolcano(res.lfc,lab = rownames(res.lfc), x = 'log2FoldChange', y = 'pvalue', xlim = c(-5, 8))
  volcano2
  # --- calcul FC lin?aire
...
DESeq2_FromSampleFiles = function(table, dir, pval, FC, analysis=c("All", "Ctrl"), condCtrl=NULL, PreFilt=NULL, 
  ...
  cat("\n Creation of the counts table")
  dds = DESeqDataSetFromHTSeqCount(Exp, directory=dir, design= ~Condition)
  Exp
  # --- pre-filtering
  if (!is.null(PreFilt))
  {
      ...
      for (i in 1:(NbLvl-1))
        {
          if (i>1)
          {
            ...
            dds = DESeqDataSetFromHTSeqCount(Exp1, directory=dir, design= ~Condition)
          }
